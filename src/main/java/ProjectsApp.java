...
public class ProjectsApp {
	...
	// create the processUserSelection method with no return type to be called in main().
	private static void processUserSelections() {
		// add local variable 'done' that will determine a boolean data type to reflect status.
		boolean done = false;
		// while status 'done' is false (default based on local variable), try... ; catch... an exception if occurs; 
	...
	private static int getUserSelection() {
		// print menu
		printOperations();
		// take in user input. distinct name for method call. This method will return int value.
		int input = getIntInput("Enter a menu selection");
		// return that checks value of local variable input is null.
		// Objects.isNull checks the Integer object is null. the Integer value 'input' is passed into the isNull method.
		// ? is input null? return -1, if its not, return the input value.
		return Objects.isNull(input) ? -1 : input;
	}
	private static void printOperations() {
		 System.out.println("\nThese are the available selections. Press the Enter key to quit: ");
		 // run forEach method on operationsList to iterate through the List operationsList.
		 // forEach thing -> print "  " then the thing. thing can be anything. when forEach is called,
	...
	private static String getStringInput(String prompt) {
		// keep the cursor in-line; NOTE print not println. println terminates line, do not use.
		System.out.print(prompt + ": ");
		// take user input.
		String input = userInput.nextLine();
		// input String is blank? return null, otherwise run trim on input to trim whitespace, outer whitespace not removed.
